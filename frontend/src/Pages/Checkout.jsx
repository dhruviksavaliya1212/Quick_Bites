...
const Checkout = () => {
  ...
  // save address from list when place order
  ...
  const cashOnDelivery = async () => {
    ...
    let orderData = {
      address:Address,
      items:orderItems,
      amount:getTotalCartAmount()+calculateDelivery()+calculateGst()+calculatePlatformFee() 
    }

    const {data} = await axios.post(`${backend}/api/order/cash-on-delivery`,{orderData},{headers:{token}});
  ...
  const stripePayment = async()=>{
    ...
    const {data} = await axios.post(`${backend}/api/order/online-stripe`, {orderData}, {headers:{token}});
  ...
  const initPay = async (order) => {
    const options = {
      ...
      handler: async(response)=>{
        try {
          const {data} = await axios.post(`${backend}/api/order/verify-razorpay`,{response},{headers:{token}});
  ...
  const paymentRazorpay = async() => {
    ...
    const {data} = await axios.post(`${backend}/api/order/online-razorpay`,{orderData},{headers:{token}});
          ...
          {listOfAdd.map((item, index) => (
            ...
            >
              <img
                ...
                alt=""
                className={`w-8 absolute top-4 left-20  ${
                  Address === item ? "flex" : "hidden"
                }`}
              />
              <img
